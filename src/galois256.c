
#include <stdint.h>

const uint8_t galois_256_anti_logs[256] = {
        
};

const uint8_t galois_256_logs[256] = {
        
};

// section 2.4 Finite Field Multiplication Using Tables
// http://techheap.packetizer.com/cryptography/encryption/spec.v36.pdf
uint8_t galois_256_multiplication_look_up(uint8_t a, uint8_t b) {
    uint16_t t = 0;

    if (a == 0 || b == 0) {
        return 0;
    }
    t = galois_256_logs[a] + galois_256_logs[b];
    if (t > 0xff) {
        t = t - 0xff;
    }

    return galois_256_anti_logs[t];
}

// section 2.4 Finite Field Multiplication Using Tables
// http://techheap.packetizer.com/cryptography/encryption/spec.v36.pdf
uint8_t galois_256_inverse_look_up(uint8_t a) {
    if (a == 0) {
        return 0;
    }

    return galois_256_anti_logs[0xff - galois_256_logs[a]];
}

static void swap_rows(uint8_t *a, const int i, const int pivot, const int cols) {
    int j;
    uint8_t tmp;
    for (j = 0; j < cols; j++) {
        tmp = a[pivot * cols + j];
        a[pivot * cols + j] = a[i * cols + j];
        a[i * cols + j] = tmp;
    }
}

void
galois_256_matrix_row_reduced_echelon_form(uint8_t *matrix, const int nrows, const int ncols, const int start_col) {
    int i, j, col, row, pivot;
    uint8_t scale;

    row = 0;
    for (col = start_col; col < ncols && row < nrows; col++) {
        pivot = row;

        for (i = row + 1; i < nrows; i++) {
            if (matrix[i * ncols + col] > matrix[pivot * ncols + col]) {
                pivot = i;
            }
        }

        if (matrix[pivot * ncols + col] == 0) {
            continue;
        }

        if (row != pivot) {
            swap_rows(matrix, row, pivot, ncols);
        }

        for (i = row + 1; i < nrows; i++) {
            scale = galois_256_multiplication_look_up(matrix[i * ncols + col],
                                                      galois_256_inverse_look_up(matrix[row * ncols + col]));
            for (j = col + 1; j < ncols; j++) {
                matrix[i * ncols + j] ^= galois_256_multiplication_look_up(matrix[row * ncols + j], scale);
            }

            matrix[i * ncols + col] = 0x00;
        }

        if (matrix[row * ncols + col] != 0x00) {
            for (i = ncols - 1; i > col - 1; i--) {
                matrix[row * ncols + i] = galois_256_multiplication_look_up(matrix[row * ncols + i],
                                                                            galois_256_inverse_look_up(
                                                                                    matrix[row * ncols + col]));
            }

            for (i = 0; i < row; i++) {
                for (j = ncols - 1; j > col - 1; j--) {
                    matrix[i * ncols + j] ^= galois_256_multiplication_look_up(matrix[i * ncols + col],
                                                                               matrix[row * ncols + j]);
                }
            }
        }

        row++;
    }
}

void galois_256_vector_matrix_multiplication(const uint8_t *vector, const int vector_size, const uint8_t *matrix,
                                             const int matrix_nrows, const int matrix_ncols, uint8_t *result) {
    int i, j;
    // look up multiplication
    for (i = 0; i < vector_size; i++) {
        for (j = 0; j < matrix_nrows; j++) {
            result[i] ^= galois_256_multiplication_look_up(vector[j], matrix[j * matrix_ncols + i]);
        }
    }
}