...
int AES_set_decrypt_key(const unsigned char *user_key, const int bits, AES_KEY *key) {
    ...
    status = AES_set_encrypt_key(user_key, bits, key);
    if (status < 0)
        return status;
...
void AES_encrypt(const unsigned char *in, unsigned char *out, const AES_KEY *key, AES_SBOX_INPUTS *sbox_inputs) {
    ...
    r = key->rounds >> 1;
    sbox_inputs->rounds = key->rounds;
    si = 0;

    for (;;) {
        PUTU32((sbox_inputs->sbox_inputs + si), s0);
        PUTU32((sbox_inputs->sbox_inputs + +si + 4), s1);
        PUTU32((sbox_inputs->sbox_inputs + +si + 8), s2);
        PUTU32((sbox_inputs->sbox_inputs + +si + 12), s3);
        si += 16;
        t0 =
                Te0[(s0 >> 24)] ^
                Te1[(s1 >> 16) & 0xff] ^
        ...
        PUTU32((sbox_inputs->sbox_inputs + si), t0);
        PUTU32((sbox_inputs->sbox_inputs + +si + 4), t1);
        PUTU32((sbox_inputs->sbox_inputs + +si + 8), t2);
        PUTU32((sbox_inputs->sbox_inputs + +si + 12), t3);
        si += 16;
        s0 =
                Te0[(t0 >> 24)] ^
                Te1[(t1 >> 16) & 0xff] ^
